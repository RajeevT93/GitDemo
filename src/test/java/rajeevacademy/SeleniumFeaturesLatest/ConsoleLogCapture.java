...
public class ConsoleLogCapture {
	...
	public static void main(String[] args) {
		...
		LogEntries entry = driver.manage().logs().get(LogType.BROWSER); //get (LogEntries(class)) Objects
		List <LogEntry> logs = entry.getAll(); //LogEntries Object(entry)-getAll method return all logs in list
		
		for ( LogEntry e :logs) { //iterating through list and printing each Log
			System.out.println(e.getMessage()); //extract the file in Log4j
		}
